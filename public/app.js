const socket = new io("https://chat-app-deploy-bk03.onrender.com/");
...
socket.on("message", (data) => {
    ...
    if (name === nameInput.value) {
        if(isSameName){
            ...
            li.innerHTML = `<div class="post--user post__text">${text}</div>`
        }
        else{
            li.className = "post--right";
        ...
        if(isSameName){
            ...
            li.innerHTML = `<div class="post--reply post__text">${text}</div>`;
        }
        else{
            li.className = "post--left";
            li.innerHTML = `<div class="post__header">
